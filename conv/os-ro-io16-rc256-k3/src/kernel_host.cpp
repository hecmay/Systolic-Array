...
int main(int argc, char **argv){
 ...
 
  {
    ...
    std::vector<float, aligned_allocator<float>> dev_cin_unserialized((130) * (130) * (16));
    std::vector<float, aligned_allocator<float>> dev_cin(1081344);
    std::vector<float, aligned_allocator<float>> dev_cout_unserialized((128) * (128) * (16));
    std::vector<float, aligned_allocator<float>> dev_cout(262144);
    std::vector<float, aligned_allocator<float>> dev_w_unserialized((16) * (3) * (3) * (16));
    std::vector<float, aligned_allocator<float>> dev_w(73728);

    // Initialize host buffers
    std::copy(reinterpret_cast<float *>(cin), reinterpret_cast<float *>(cin) + (130) * (130) * (16), dev_cin_unserialized.begin());
    std::copy(reinterpret_cast<float *>(cout), reinterpret_cast<float *>(cout) + (128) * (128) * (16), dev_cout_unserialized.begin());
