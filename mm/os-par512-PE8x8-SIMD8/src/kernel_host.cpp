...
//  data_t A[I][K], B[K][J], C[I][J], C_golden[I][J]; 
...

  {
    ...
    std::vector<int, aligned_allocator<int>> dev_A_unserialized((4096) * (4096));
    std::vector<int, aligned_allocator<int>> dev_A(134217728);
    std::vector<int, aligned_allocator<int>> dev_B_unserialized((4096) * (4096));
    std::vector<int, aligned_allocator<int>> dev_B(134217728);
    std::vector<int, aligned_allocator<int>> dev_C_unserialized((4096) * (4096));
    std::vector<int, aligned_allocator<int>> dev_C(16777216);

    // Initialize host buffers
    std::copy(reinterpret_cast<int *>(A), reinterpret_cast<int *>(A) + (4096) * (4096), dev_A_unserialized.begin());
    std::copy(reinterpret_cast<int *>(B), reinterpret_cast<int *>(B) + (4096) * (4096), dev_B_unserialized.begin());
